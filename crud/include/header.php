<!doctype html> <!-- O <!doctype html> é uma declaração para informar qual a verção do HTML está sendo utilizado no arquivo, neste caso está sendo representato o HTML 5, a versão mais nova do HTML. -->
<html lang="pt-br"> <!-- O atributo 'lang' define para o navegador a linguagem de determinado bloco HTML, no exemplo, especifica que o conteudo inteiro dentro da tag html esta em portugues -->
  <head> <!-- A tag <head> providencia informações gerais (metadados) sobre o documento, incluindo seu titulo e links para scripts e folhas de estilos -->
    <meta charset="utf-8"> <!-- O atributo 'charset' especifica qual a codificação de caracteres utilizada na página -->
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no"> <!-- a tag meta com o atributo 'viewport' junto ao atributo 'content' define o tamanho da janela de visualização da página -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">
    <title>Vagas</title>
  </head>
  <body class="bg-dark text-light"> 
<div class="container">

<div class="jumbotron bg-danger">
    <h1>Vagas</h1>
    <p>Exemplo de CRUD com PHP orientado a objetos</p>
</div>

